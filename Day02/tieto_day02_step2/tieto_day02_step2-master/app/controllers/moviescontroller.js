(function(){
	...
	var movieFun = function($scope, $routeParams){
		$scope.selectedHero = $routeParams.id;
		$scope.heores = [{
		"id" : 1,
		"movies" : 3,
		"title" : "Batman",
		...
		$scope.movies = $scope.heores[$scope.selectedHero-1].movieslist;
		$scope.backFun = function(){
			history.back();
		}
	};
	
	angular.module("app").controller("moviescontroller",movieFun);
